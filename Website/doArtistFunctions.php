<?php
	...
	function AddHangingFees() {
		global $db;
		
		$year = date("n") >= 3 ? date("Y") + 1: date("Y");
		$result = $db->query("SELECT ap.ArtistAttendingID, ad.IsEAP FROM ArtistPresence ap INNER JOIN ArtistDetails ad ON " .
			"ad.ArtistID = ap.ArtistID WHERE ap.Year = $year AND PeopleID = " . $_SESSION["PeopleID"]);
		...
		if($result->num_rows > 0)
		{
			...
			$db->query("INSERT INTO ShoppingCart (ItemTypeName, ItemTypeID, ItemDetail, PurchaserID, Price, Created) VALUES (" . 
				"'HangingFees', $attendingID, '$count Piece" . ($count == 1 ? "" : "s") . " of Art', " . $_SESSION["PeopleID"] . 
				", $fees, NOW())");
	...
	elseif($task == "AddArtShowItem")
	{
		...
		$sql = "INSERT INTO ArtSubmissions (ArtistAttendingID, ShowNumber, Title, Notes, IsPrintShop, IsOriginal, OriginalMedia, " .
			"PrintNumber, PrintMaxNumber, MinimumBid, QuickSalePrice, FeesPaid) VALUES ($attendingID, $showNumber, '$title', '$notes', " .
	...
	elseif($task == "UpdateStatus")
	{
		...
		{
			$result = $db->query("SELECT CONCAT(p.FirstName, ' ', p.LastName) AS Name, p.Email, ap.Status, ap.StatusReason FROM People p INNER JOIN " .
				"ArtistDetails ad ON p.PeopleID = ad.PeopleID INNER JOIN ArtistPresence ap ON ap.ArtistID = ad.ArtistID WHERE " .
				"ap.ArtistAttendingID IN ($requests) AND p.Email != ''");
			if($result->num_rows > 0)
			{
				while($row = $result->fetch_array())
				{
					...
					if($action == "Approved")
					{
						...
						$mail->Body = "Hello! This email is to let you know that your request to show art at the next Capricon Art " . 
							...
							"with the approval: \"$thisReason\"." : "") . "\r\n\r\nYou may now begin to fill in artwork that you will " .
							"be bringing or sending to the Art Show by going to https://registration.capricon.org/artistSubmissions.php " .
							"and adding pieces being shown in the main show or to be sold in the Print Shop. Please note that there is " .
							"a nominal Hanging Fee that needs to be paid on anything shown in the Art Show (not the Print Shop) based on " .
							"the minimum bid. See the Art Submission page for details. If you have any questions, please feel free to " . 
