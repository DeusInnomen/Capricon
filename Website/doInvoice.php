	...
	if(isset($_REQUEST["action"]))
	{
		switch($_REQUEST["action"])
		{
			case "CreateInvoices":
				...
				//We'll pull any outstanding things that need invoicing:
				//Unsold Art Show Items - Pricing with Taxes
				//Dealers - Dealer Fee, Badge or badge packages,
				//Paypal and Stripe integration (Below) to automatically invoice process.
				//Email them changes -
				
				//Unsold Art Show Items
				$sql = "SELECT ArtID, Title, OriginalMedia, " .
				...
				$sql = "SELECT Title as B_Address1 FROM ArtSubmissions WHERE " . 
					" QuantitySold<1 or QuantitySold IS NULL";
					
				$in_data = sql_to_array($sql);
				...
				*/
				...
				//Forced insert has no keys
				$out_array = update_from_array($in_data,"invoice_header","",null,"headerID");
				
				echo "<pre>";
				print_r($out_array);
				echo "</pre>";
				
				echo '{ "success": true, "message": "Invoices Generated" }';
				
				//Old Data - in case we need it.
				/*
